<?php
 // Wicked Software Application Manager class 0.3.9
 // --> Desc.: This is the god of all classes, it manages all the other ones.
 // --> Dependencies: Display 0.5.0
 //                   dbMySQL 0.8.0
 //                   log 0.3.1
 //                   file 0.2.1
 // (c) 2011-2013 Wicked Software
 ...
 class wsam { 
  ...
  private $pkg_dependencies = array ( 0 => array ( 0 => 5, "name" => "", "ver" => "", "sublocation" => "", "hash" => "" ),
                                      1 => array ( 0 => 0, "name" => "dbmysql", "ver" => "0.8.6", "sublocation" => "",
                                                   "hash" => "" ),
                                      2 => array ( 0 => 0, "name" => "display", "ver" => "0.5.4", "sublocation" => "",
                                                   "hash" => "" ),
                                      3 => array ( 0 => 0, "name" => "log", "ver" => "0.3.2", "sublocation" => "",
                                                   "hash" => "" ),
                                      4 => array ( 0 => 0, "name" => "file", "ver" => "0.4.3", "sublocation" => "",
                                                   "hash" => "" ),
                                      5 => array ( 0 => 0, "name" => "session", "ver" => "0.4.3", "sublocation" => "",
                                                   "hash" => "" ) );
  private $pkg_lastver = "0.3.8";    
  private $pkg_released = "22/06/2013";  
  private $pkg_copyright = "(c) 2013 Wicked Software";
  ...
  private function selfstart() {
   ...
   if ($this->loadconfig()) {
    ...
    if ($wsam_conf['app']['installed']) {
     ...
     $this->startsql();
     $this->checkmasterdb();
     $this->db->query("SELECT * FROM `".$this->dbtable."` LIMIT 1;");
...
//     }
  ...
  private function addtomasterdb() {
   ...
   return $this->db->query("INSERT INTO `".$this->dbtable."` ( `major` , `minor` , `revision` ) VALUES ( '".$ver[0]."',  '".$ver[1]."',  '".$ver[2]."' );");
  }
  
  private function existmasterdb() {
   $this->log(":WSAM: Checking if the master database exist..");
   if ($this->db->exist_table($this->dbtable)) {
  ...
  private function checkmasterdbstructure() {
   ...
   $retcode = 1;
   $this->db->query("DESCRIBE ".$this->dbtable.";");   
   $sql = $this->db->fetch_array();
   $result = array ( array() );
